...
>(({ children, ...props }, ref) => {
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Check for data-no-toggle attribute first (most specific)
    if (target.closest('[data-no-toggle]')) {
      return
    }
    
    // Check if clicking directly on or inside interactive elements
    const isInteractive = 
      ...
      target.tagName === 'A' ||
      target.closest('svg')?.closest('button') // Handle SVG icons inside buttons
    
    if (isInteractive) {
      return
    }
    
    // Allow toggle for other clicks
    toggleExpand()
  }
  
  return (
    <div ref={ref} onClick={handleClick} className="cursor-pointer" {...props}>
      {children}
    </div>
  )
})
